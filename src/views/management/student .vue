...
export default {
  ...
  methods: {
     ...
     let that = this;
      that.axios
        .post("/api/Student/AddStudent", {
          ...
          stuClassId: that.characters,//班级编号
        ...
        .then(function(res) {
          ...
          if (res.data.code == 1) {
            ...
            that.characters = "";
            that.student = []; //新增成功后清空新增框内的值
            that.stuDent(); //重新调用获取班级信息方法给tableData赋值
          }
        })
        .catch(err => {
          console.log(err);
    ...
    handleEdit(index, row) {
      ...
      // that.characters = that.tableData[index].userUserTypeId;
    ...
    submitForm(formName) {
      ...
      that.axios
        .post("/api/Student/ModifyStudent", {
          ...
          stuBirthDay: that.student.birthDay,
          stuClassId: that.tableData[that.index].classId,
